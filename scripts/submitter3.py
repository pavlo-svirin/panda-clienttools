...
class SequentialLQCDSubmitter:
	...
	repr_cmd = "perl /Users/pavlosvirin/dev/panda-submission2/scripts/graph-easy.pl"
	...
	def createJob(self, name, nodes, walltime, command, inputs = None, queuename = None, environment=None, stdin=None):
		job = JobSpec()
		...
		if environment is not None:
			lqcd_command['environment'] = environment
		if stdin is not None:
			lqcd_command['stdin'] = stdin
	...
	def __repr__(self):
		...
		for i in self.__joblist:
			if i.cmtConfig is not None:
				...
				if js['next'] is not None and any(js['next']):
                                    for jn in js['next']:
					routes.append("[{0}]->[{1}]".format(js['name'], jn))
				else:
...
for jobname, descr in jobdef['jobs'].items():
	...
	#print jobname
	job = sls.createJob(name=jobname, walltime=descr['walltime'], command=descr['command'], nodes=descr['nodes'], inputs=None, queuename=qname, environment=environment, stdin=stdin)
	#print(job)
	#print '===============\n'
